...
var coverson = true; //requests enabled, covers on!
...
function saveViewStatus(whatviewed){
    ...
    viewdata.percentrich=percentrich;
    viewdata.timelogged=new Date().getTime();
    saveData(viewdata);
}
function saveExclusiontripped(reason){
    var excluded = {};
    excluded.exclusionreason=reason;
    excluded.id = subjectID;
    excluded.percentrich=percentrich;
    excluded.whichtrial=whichtrial;
    saveData(excluded);
}
...
function savetrial(){
    ...
    exp_data.feedbackscore=(correctcount*10)-(64-correctcount)*10;
...
//new bit: cover up unknown plankton while making requests...
	if(coverson&&knownids.indexOf(aflower.idno)<0){
...
function showtrial(){
    ...
    var buttcol="#999999";
    var posbutton = "<button id=\"posreqbutton\" onclick=\"consultDelay('pos')\" style=\"background-color:"+buttcol+"\"><p>Get a</p><p style=\"color:"+richcolor+"\"> <strong>selenoid rich</strong></p> example</button>";
    var negbutton = "<button id=\"negreqbutton\" onclick=\"consultDelay('neg')\" style=\"background-color:"+buttcol+"\"><p>Get a</p> <p style=\"color:"+poorcolor+"\"><strong> selenoid poor</strong></p> example</button>";
    scroll(0,0);
    ...
    atrial+="</tr>"
    atrial+="<tr><td colspan=\"3\"><div id=\"goonbutton\"><button id=\"submitans\" onclick=\"coversoff()\">I'm done with example requests!</button></div></td></tr>";
    atrial+="</table>";
...
function coversoff(){
    ...
    document.getElementById("posreqbutton").disabled=true;
    document.getElementById("negreqbutton").disabled=true;
    document.getElementById("goonbutton").innerHTML="<button id=\"submitans\" onclick=\"submit()\">Submit answer</button>";
    redraw();
}
function submit(){
    allactions.push("S");
    ...
    //Exclusion I: number of requests reasonable (not zero or >60)
    if(richrequests+poorrequests==0)saveExclusiontripped("zerorequests");
    if(richrequests+poorrequests>60)saveExclusiontripped("over60requests");

    //Exclusion II: labelled plankton sorted correctly. (alert if in practice trial, exclude if in real.)
	for(var i=0;i<knownids.length;i++){
		...
		else {saveExclusiontripped("badlabels"); break;}
	    }
...
//other only-enforced-in-practice checks: try all possible actions, have a plausible sort.
//    if(whichtrial==0){
//	if(richrequests==0){
...
function feedback(){
    ...
    if(correctcount>40&&correctcount<60)fdbk+="Not a bad score! But it is possible to do better...";
    ...
    document.getElementById("trialwordsdiv").setAttribute("style","height:auto");
...
function tweenscreen(){
    ...
    if(whichtrial==0)tweenmessage+="pattern 1!";
    if(whichtrial==1) tweenmessage+="pattern 2!";
